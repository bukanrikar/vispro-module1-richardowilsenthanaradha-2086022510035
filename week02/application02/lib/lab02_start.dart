...
import 'features/menu/presentation/widgets/menu_item_row.dart';
...
class _MenuScreenState extends State<MenuScreen> {
  ...
  @override
  Widget build(BuildContext context) {
    final List<MenuItem> visible = _items.where((item) {
      return _query.isEmpty || item.name.toLowerCase().contains(_query.toLowerCase());
    }).toList();
        ...
        children: [
          ...
          Expanded(
            child: visible.isEmpty
                ? MenuEmptyState(query: _query, onClearSearch: _clearSearch)
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return MenuItemRow(
                        ...
                        onIncrement: () => setState(() => _quantities[item.id] = (_quantities[item.id] ?? 0) + 1),
                        onDecrement: () => setState(() => _quantities[item.id] = (_quantities[item.id] ?? 0) - 1),
                      );
                    },
                  ),
          ),
